...
//        } while (true);
        ...
        myMajiang.add(MjHelper.发指定牌(PAI_XING.七万));
        myMajiang.add(MjHelper.发指定牌(PAI_XING.八万));
        myMajiang.add(MjHelper.发指定牌(PAI_XING.九饼));
        myMajiang.add(MjHelper.发指定牌(PAI_XING.红中));
        myMajiang.add(MjHelper.发指定牌(PAI_XING.红中));
        System.out.print("\t\t听牌牌型:" + myMajiang.stream().map(ma -> ma.getPai().name()).collect(Collectors.toList()));

        long t1 = System.currentTimeMillis();
        List<PAI_XING> list = new ArrayList<>();
        for (PAI_XING paiXing : PAI_XING.values()) {
            List<Majiang> temp = new ArrayList<>();
            temp.add(new Majiang(paiXing));
            temp.addAll(myMajiang);
            List<Majiang> result = MjHelper.验证胡牌(temp);
            if (result != null) {
                list.add(paiXing);
            }
        }
        long t2 = System.currentTimeMillis();
        if (list.size() > 0) {
            System.out.println("\t\t耗時:" + (t2 - t1) + "\t\t可胡牌牌型:" + list.stream().map(ma -> ma.name()).collect(Collectors.toList()));
        } else {
            System.out.println("\t\t耗時:" + (t2 - t1) + "\t\t未停牌");
        }
